(() => {
  ...
  function buildRules(customRegexLines = []) {
    const namePattern = /\b[\p{L}][\p{L}'’.-]{1,}(?:\s+[\p{L}][\p{L}'’.-]{1,}){1,3}\b/gu;
    const nameJoiners = new Set([
      'al', 'ap', 'bin', 'binti', 'da', 'dal', 'de', 'del', 'dela', 'della', 'dem', 'den', 'der', 'di', 'dos', 'du',
      'el', 'ibn', 'la', 'le', 'mac', 'mc', 'san', 'st', 'st.', 'van', 'von', 'zu'
    ]);
    const honorifics = new Set(['mr', 'mrs', 'ms', 'mx', 'dr', 'prof', 'sir', 'madam', 'lord', 'lady']);
    const looksLikeName = (value) => {
      const cleaned = value.trim().replace(/[.,]+$/g, '');
      const parts = cleaned.split(/\s+/).filter(Boolean);
      if (parts.length < 2) return false;
      if (parts.length > 4) return false;
      let substantialTokens = 0;
      for (const raw of parts) {
        const token = raw.toLowerCase();
        if (honorifics.has(token)) continue;
        if (nameJoiners.has(token)) continue;
        // Starts with an uppercase letter in any alphabet.
        if (/^\p{Lu}/u.test(raw)) {
          substantialTokens += 1;
          continue;
        }
      ...
      return substantialTokens >= 2;
    };
    const looksLikeInternationalPhone = (value) => {
      const digits = value.replace(/\D/g, '');
      if (digits.length < 7 || digits.length > 15) return false; // E.164 compatible window
      if (digits.length >= 13 && luhnLooksValid(digits)) return false; // avoid swallowing card-like numbers
      return true;
    };

    const rules = [
      { category: 'email', regex: /[A-Z0-9.!#$%&'*+/=?^_`{|}~-]{1,64}@[A-Z0-9-]{1,63}(?:\.[A-Z0-9-]{1,63})+/gi },
      {
        category: 'phone',
        regex: /(?:\+|00)?\d{1,3}[\s.-]?(?:\(?\d{1,4}\)?[\s.-]?){1,6}\d{1,4}\b/g,
        filter: looksLikeInternationalPhone
      },
      ...
      { category: 'card', regex: /\b(?:\d[ -]*?){13,19}\b/g, filter: luhnLooksValid },
      {
        ...
        filter: (value) => {
          ...
          return !stop.has(value) && looksLikeName(value);
        }
      }
    ];
  ...
  function redactTextWithRules(input, rules) {
    ...
    // Optional Phase-1 NLP assist for names (compromise.js), fully local when vendored.
    if (window.nlp) {
      const nlpRedacted = redactNamesWithNlp(output);
      output = nlpRedacted.text;
      counts.name += nlpRedacted.count;
    }

    return { text: output, counts };
  }

  function redactNamesWithNlp(text) {
    try {
      const doc = window.nlp(text);
      const people = doc.people().out('array') || [];
      if (!people.length) return { text, count: 0 };

      // Replace longer entities first to avoid partial replacements.
      const names = [...new Set(people.map((p) => String(p).trim()).filter(Boolean))]
      ...
      for (const name of names) {
        ...
        output = output.replace(pattern, (m) => {
          count += 1;
